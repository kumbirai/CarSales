...
/**
 * The panel used for adding cars to the CarSalesSystem @ * PUBLIC FEATURES: //
 * Constructors public AddCarPanel(CarSalesSystem carSys, JPanel dest)
 * 
...
public class AddCarPanel extends JPanel implements ActionListener {
	...
	/**
	 * @param carSys
	 *            links to a CarSalesSystem object
	 * @param dest
	 *            where the components will be placed
	 */
	...
	private boolean validateString(String arg) {
		boolean valid = false;
		String[] splitted = arg.split(" "); // splits argument around spaces and
											// creates an array

		for (int i = 0; i < splitted.length; i++) {
			// checks if the number of characters between a space is greater
			// than 2
	...
	/**
	 * checks the argument It is valid if it contains a decimal value, with only
	 * one decimal place
	 * 
	 * @param distance
	 *            a double value expressed in a string
	 * @return true if valid, false otherwise
	 */
