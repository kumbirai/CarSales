...
/**
 * Stores manufacturers objects, and performs searches @ * PUBLIC FEATURES: //
 * Constructors public CarsCollection() public CarsCollection(Manufacturer man)
 * 
...
public class CarsCollection {
	...
	/**
	 * @param man
	 *            manufacturer object to add to collection
	 */
	...
	/**
	 * adds a car to a CarCollection and files it in an appropriate
	 * manufacturer, or creates a new manufacturer if none exist for the car
	 * 
	 * @param c
	 *            car to add to collection
	 * @return either one of NO_ERROR, CARS_MAXIMUM_REACHED, or
	 *         MANUFACTURERS_MAXIMUM_REACHED
	 */
	public int addCar(Car c) {
		Manufacturer man;
		String name = c.getManufacturer().getManufacturer();
		int index = -1;
		int result = NO_ERROR;

		for (int i = 0; i < manufacturer.length; i++) {
			// if manufacturer already exists
			if (manufacturer[i].getManufacturer().equalsIgnoreCase(name))
				index = i;
		}
	 ...
	 * 
	 * @return entire collection of cars in CarsCollection from all
	 *         manufacturers
	 */
	public Car[] getAllCars() {
		...
		List<Car> cars;
		for (int i = 0; i < manufacturer.length; i++) {
			cars = manufacturer[i].getCars();
			result.addAll(cars);
		}

		return CarSalesSystem.listToCar(result);
	}
	 ...
	 * 
	 * @return value indicating the average age of all the cars in the
	 *         collection
	 */
	...
	/**
	 * calculate the average distance traveled from the entire collection of
	 * cars
	 * 
	 * @return value indicating the average distance travelled of all the cars
	 *         in the collection
	 */
	 ...
	 * 
	 * @return value indicating the average price of all the cars in the
	 *         collection
	 */
	public double getAveragePrice() {
		...
		for (int i = 0; i < manufacturer.length; i++) {
			cars = manufacturer[i].getCars();
			for (Car car : cars) {
				result = car.getPrice().doubleValue() + result;
				count++;
			}
		}
		if (count == 0)
	...
	/*
	 * public Manufacturer getManufacturer(int index) { Manufacturer
	 * returnManufacturer;
	 * 
	 * try { returnManufacturer = manufacturer[index]; } catch (Exception exp) {
	...
	/**
	 * load entire collectoin of cars into the manufacturer object from a data
	 * file
	 * 
	 * @param file
	 *            filename of binary file to load car data from
	 */
	 ...
	 *            array to resize
	 * @param extendBy
	 *            indicates how many elements should the array be extended by
		...
		if (manufacturer.length > 0) {
			do {
				...
				for (int i = 0; i < items; i++) {
					if (i + 1 < items) {
						if (manufacturer[i].getManufacturer().compareTo(manufacturer[i + 1].getManufacturer()) > 0) {
							temp = manufacturer[i];
			...
			for (Manufacturer man : manufacturer) {
				Manufacturer entity;
	...
	public Car[] search(int minAge, int maxAge) {
		...
		/*
		 * Putting the if statement first will increase effeciency since it
		 * isn't constantly checking the condition for each loop. It does use
		 * almost double the amount of code though
		 */
		if (maxAge == -1) {
			for (int i = 0; i < car.length; i++) {
