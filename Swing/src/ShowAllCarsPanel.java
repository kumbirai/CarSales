...
public class ShowAllCarsPanel extends JPanel implements ActionListener, ChangeListener {
	...
	/**
	 * when the tab on the main frame gets changed over to this one, we may have
	 * to update the car list with the latest version
	 * 
	 * @param ev
	 *            ChangeEvent object
	 */
	public void stateChanged(ChangeEvent ev) {
		...
		if (ev.getSource() instanceof JTabbedPane) {
			...
			// if selected tab index is the ShowAllCarsPanel, which is this
			// panel
			if (tab.getSelectedIndex() == 2) {
				...
				if (carsUpdated) {
					...
					// next time don't bother updating the car list unless a car
					// is added again
					carsUpdated = false;
				}
			}
	...
	/**
	 * display previous item in index, otherwise display no next item message
	 * dialog
	 */
